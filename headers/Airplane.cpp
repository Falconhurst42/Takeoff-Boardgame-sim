...
vector<Airplane*>& Airplane::get_occupancy_special(Airport* loc, bool main_board) {
    if(main_board) {
...
Airport*& Airplane::get_location_special(bool main_board) {
    if(main_board) {
        return location;
    }
    else {
...
bool Airplane::check_bump(bool main_board) {
...
// Accessors
string Airplane::get_loc_name(bool main_board) {
    return get_location_special(main_board)->name;
}

Airport* Airplane::get_loc(bool main_board) {
    return get_location_special(main_board);
}

const Player* Airplane::get_owner_ptr() {
    return owner;
}
